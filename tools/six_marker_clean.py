#!/usr/bin/env python3
"""
666 MARKER ANALYZER - Clean Version
...
def find_more_666_markers():
    ...
    pos_977 = None
    for i, (_, c, _) in enumerate(markers_found):
        if c == 977:
            pos_977 = i + 1
            break
    
    if pos_977:
        print(f"\n    977 ist Marker #{pos_977} in der Liste der 6-X-6 Primzahl-Triple")
    ...
    print(f"    Dichte: etwa 1 in {2000 // len(markers_found) if len(markers_found) > 0 else 'inf'} Zahlen")
...
das als versteckter 666-Marker interpretiert werden KANN.
Die doppelte 6 (links und rechts) koennte symbolisch
fuer 666 stehen (6*111).

Ob es BEWUSST als Marker gewaehlt wurde, bleibt
